...
impl Tty {
    ...
    fn device_added(
        ...
        let mut try_initialize_gpu = || {
            let mut last_err = anyhow::anyhow!("EGL initialization failed");

            // Retry waking up the GPU up to 5 times (for slow ACPI transitions from D3cold on startup)
            for attempt in 1..=5 {
                let res = (|| -> Result<DrmNode, anyhow::Error> {
                    ...
                    if egl_device.is_software() {
                        return Err(anyhow::anyhow!("SOFTWARE_RENDERER"));
                    }
                ...
                match res {
                    Ok(render_node) => return Ok(render_node),
                    Err(err) => {
                        if err.to_string() == "SOFTWARE_RENDERER" {
                            return Err(anyhow::anyhow!("software EGL renderers are skipped"));
                        }
                        if attempt < 5 {
                            warn!("failed to initialize GPU node (attempt {attempt}/5), retrying in 50ms: {err:?}");
    ...
    pub(crate) fn check_suspend_device(&mut self, node: DrmNode) -> bool {
        let mut topology_changed = false;

        let should_suspend = if let Some(device) = self.devices.get(&node) {
            node != self.primary_node
                && device.surfaces.is_empty()
                && device.active_leases.is_empty()
                && !device.is_suspended
                && device.render_node.is_some()
                && !self.config.borrow().debug.disable_idle_drm_device_suspend
        } else {
            false
        };

        if should_suspend {
            let render_node = self.devices.get(&node).unwrap().render_node.unwrap();
            // Multiple primary nodes can share a render node (see device_removed()); only
            // remove it when no other (non-suspended) device still uses it.
            ...
            if was_last {
                debug!("secondary GPU {node:?} has no active outputs/leases, suspending device to allow power management");
                ...
                debug!("secondary GPU {node:?} has no active outputs/leases, but render node is still in use; skipping suspend");
            }

            // Mark this primary node as suspended even if the render node stays active due to sharing.
            self.devices.get_mut(&node).unwrap().is_suspended = true;
        }

        topology_changed
    }

    pub(crate) fn resume_device(&mut self, node: DrmNode) -> Result<bool, ()> {
        let mut topology_changed = false;
        if let Some(device) = self.devices.get(&node) {
            if device.is_suspended {
                debug!("resuming suspended GPU device {node:?}");
                let render_node = device.render_node;
                let gbm = device.gbm.clone();

                let shared_active = render_node.is_some()
                    && self.devices.iter().any(|(other_node, other)| {
                        *other_node != node
                            && other.render_node == render_node
                            && !other.is_suspended
                    });

                if shared_active {
                    debug!("resuming suspended GPU device: {node:?} (render node still active)");
                    self.devices.get_mut(&node).unwrap().is_suspended = false;
                } else if let Some(render_node) = render_node {
                    // Retry waking up the GPU up to 3 times (for slow ACPI transitions)
                    let resumed = (1..=3).any(|attempt| {
                        if self.gpu_manager.as_mut().add_node(render_node, gbm.clone()).is_ok() {
                            return true;
                        }
                        warn!("failed to resume GPU node (attempt {attempt}/3)");
                        if attempt < 3 {
                            std::thread::sleep(std::time::Duration::from_millis(50));
                        }
                        false
                    ...
                    if !resumed {
                        error!("failed to resume GPU node {node:?} after 3 attempts");
                        return Err(());
                    }

                    let _ = self.gpu_manager.devices(); // Trigger re-enumeration
                    self.devices.get_mut(&node).unwrap().is_suspended = false;
                    topology_changed = true;
                } else {
                    self.devices.get_mut(&node).unwrap().is_suspended = false;
    ...
    pub fn on_output_config_changed(&mut self, niri: &mut Niri) {
        ...
        for (node, connector, crtc, _name) in to_connect {
            // Eagerly wake up the GPU if it was suspended, because we are about to need its renderer!
            match self.resume_device(node) {
                Ok(true) => gpu_topology_changed = true,
                Ok(false) => {}
                Err(_) => continue, // Skip initializing output if resume failed
            }
        ...
        for node in nodes {
            if self.check_suspend_device(node) {
                gpu_topology_changed = true;
            }
        }
