        ...
        if let Some(open) = &self.open_animation {
            if !should_block_out {
                ...
                for (popup, offset) in PopupManager::popups_for_surface(surface) {
                    ...
                    let popup_alpha = alpha * popup_rules.opacity.unwrap_or(1.).clamp(0., 1.);
                ...
                if !elements.is_empty() {
                    // Use encompassing_geo to account for popups that extend
                    // beyond the main surface bounds.
                    let geo_size = encompassing_geo(scale, elements.iter())
                        .size
                        .to_f64()
                        .to_logical(scale);
