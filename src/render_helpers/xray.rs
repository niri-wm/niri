    ...
    pub fn render(
        ...
        if background.prepare(ctx.renderer, blur) {
            ...
            for (ws_geo, bg_color) in &self.workspaces {
                ...
                let elem = XrayElement {
                    ...
                    clip_geo_size,
                    corner_radius,
                    scale: params.scale as f32 * zoom as f32,
                    blur,
                    noise,
                    saturation,
                    refraction,
        ...
        if backdrop.prepare(ctx.renderer, blur) {
            ...
            let elem = XrayElement {
                ...
                noise,
                saturation,
                refraction: refraction * zoom as f32,
...
impl XrayElement {
    fn compute_uniforms(&self) -> [Uniform<'static>; 9] {
        [
            ...
            Uniform::new(
                ...
                [
                    self.refraction,
                    self.refraction_bevel,
                    self.refraction_saturation,
                    self.refraction_brightness,
                ],
            ),
            Uniform::new("effect_params", [self.feather, self.dim]),
        ]
    }
}
