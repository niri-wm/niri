...
impl State {
    pub fn layer_shell_handle_commit(&mut self, surface: &WlSurface) -> bool {
        ...
        // Arrange layers before handling commits so initial configures and geometry are up-to-date.
        map.arrange();
        ...
        if is_mapped(surface) {
            ...
            if let Some(idx) = self
                ...
                .closing_layers
                .iter()
                .position(|closing| closing.matches_surface(layer))
            {
            ...
            // geometry here is post-arrange (arrange() runs unconditionally above at
            // line 125). Clients that zero their size on teardown may produce a 0-size
            // geometry; ClosingLayer::new handles this via a snapshot-size fallback.
            let geo = map.layer_geometry(layer);
            if let Some(mapped) = self.niri.mapped_layer_surfaces.remove(layer) {
    ...
    fn start_close_animation_for_layer(
        ...
        self.backend.with_primary_renderer(|renderer| {
            ...
            match res {
                Ok(closing) => {
                    self.niri.closing_layers.push(closing);
                }
...
fn add_mapped_layer_pre_commit_hook(layer: &LayerSurface) -> HookId {
    ...
    add_pre_commit_hook::<State, _>(layer.wl_surface(), move |state, _dh, surface| {
        ...
        if should_snapshot {
            // Snapshot strategy (precedents: Hyprland onUnmap, MangoWC unmaplayersurfacenotify):
            // capture once at unmap — the last live buffer is still attached at pre-commit time —
            // and never refresh.  On content commits, skip if a snapshot already exists (first-only
            // guard) for zero steady-state cost.
            for mapped in state.niri.mapped_layer_surfaces.values_mut() {
                if mapped.surface().wl_surface() == surface {
                    let need_snapshot = got_unmapped || !mapped.has_non_empty_unmap_snapshot();
                    if need_snapshot {
