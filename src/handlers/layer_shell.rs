...
fn add_mapped_layer_pre_commit_hook(layer: &LayerSurface) -> HookId {
    // NOTE: The hook closures below do an O(n) scan of mapped_layer_surfaces to find the
    // entry matching `surface`. The hook API doesn't support capturing an identity key for
    // direct lookup. With typical panel counts this is negligible; if it becomes a concern,
    // store the LayerSurface clone or a dedicated key in the closure for O(1) lookup.
    add_pre_commit_hook::<State, _>(layer.wl_surface(), move |state, _dh, surface| {
        ...
        if should_snapshot {
            // Snapshot strategy (precedents: Hyprland onUnmap, MangoWC
            // unmaplayersurfacenotify): capture once at unmap — the last
            // live buffer is still attached at pre-commit time — and never
            // refresh.  On content commits, skip if a snapshot already
            // exists (first-only guard) for zero steady-state cost.
            for mapped in state.niri.mapped_layer_surfaces.values_mut() {
                if mapped.surface().wl_surface() == surface {
