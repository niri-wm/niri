...
impl DrmLeaseHandler for State {
    ...
    fn lease_request(
        ...
        if tty.resume_device(node).unwrap_or(false) {
            tty.update_dmabuf_feedbacks(&mut self.niri);
        }
        ...
        if res.is_err() {
            if self.backend.tty().check_suspend_device(node) {
                self.backend.tty().update_dmabuf_feedbacks(&mut self.niri);
            }
        }
